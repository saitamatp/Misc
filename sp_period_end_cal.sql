...
select sum(amount_paid) as total ,period_id,paid_id from ledger.expense_detail
group by period_id,paid_id;

create temporary table credit as 
select sum(amount_paid) as total ,period_id,paid_id from ledger.misc_credit
group by period_id,paid_id;
...
where a.period_id in (select Period_id from ledger.period_detail c where curdate() between c.Starting_from and c.Ending_on)
